...
export default function EventLocation() {
  ...
  useEffect(() => {
    async function getLatLon() {
      try {
        ...
        const { data } = await axios.get(`https://api.openweathermap.org/geo/1.0/direct?q=${eventLocation}&limit=50&appid=${import.meta.env.VITE_API_KEY}`)
        setOptions(data)
      } catch (error) {
  ...
  function handleLocation(option) {
    ...
    const saveData = ({ ...formData, eventLocation: option.name, lat: option.lat, lon: option.lon })
